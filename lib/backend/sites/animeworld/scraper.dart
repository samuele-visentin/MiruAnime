...
class AnimeWorldScraper {
  ...
  Future<Response<String>> getPage(final String url, final Map<String, String> header) async {
    ...
    if (response.statusCode! >= 300 && response.statusCode! < 400) {
      if (response.headers[HttpHeaders.locationHeader] != null) {
        String newUrl = response.headers[HttpHeaders.locationHeader]![0];
        if (!newUrl.contains(AnimeWorldEndPoints.hostname)) {
          newUrl = AnimeWorldEndPoints.sitePrefixNoS + newUrl;
        }
        final uri = Uri.parse(newUrl).replace(scheme: 'https');
  ...
  Future<AnimeWorldHomePage> getHomePage() async {
    ...
    final mainContent = page.querySelectorAll('.hotnew > div.widget-body > div.content').map((final tab) {
    ...
    final topAnime = page.querySelectorAll('.ranking > div.widget-body > div.content').map((final topSection) {
      final list = <Anime>[];
      final firstAnime = topSection.querySelector('div.item-top')!;
      list.add(Anime(
          thumbnail: firstAnime.querySelector('a.thumb > img')!.attributes['src']!,
          link: AnimeWorldEndPoints.sitePrefixNoS + firstAnime.querySelector('a.name')!.attributes['href']!,
          title: firstAnime.querySelector('a.name')!.text.trim(),
          rank: firstAnime.querySelector('i.rank')!.text.trim()));
      list.addAll(topSection.querySelectorAll('div.item').map((final element) => Anime(
          thumbnail: element.querySelector('img')!.attributes['src']!,
          title: element.querySelector('div.info > a.name')!.text.trim(),
          link: AnimeWorldEndPoints.sitePrefixNoS + element.querySelector('div.info > a.name')!.attributes['href']!,
    ...
    final RegExp regex = RegExp(
        ...
        multiLine: true);
    final sliders = page
        .querySelectorAll('#swiper-container > div.items.swiper-wrapper > div.item.swiper-slide')
        .map((final element) => Anime(
            title: element.querySelector('a.name')!.text.trim(),
            link: AnimeWorldEndPoints.sitePrefixNoS + element.querySelector('a.name')!.attributes['href']!,
    ...
    for (final anime in page
        ...
        .querySelectorAll('div.widget-body > div.film-list > div.owl-carousel > div.item')) {
      final name = anime.querySelector('a.name')!;
      ongoing.add(Anime(
        thumbnail: anime.querySelector('img')!.attributes['src']!,
        title: name.text.trim(),
    ...
    final newAdded =
        page.querySelectorAll('.simple-film-list > div.widget-body > div.item').map(getAnime).toList(growable: false);
    final upcoming = page
    ...
    return AnimeWorldHomePage(
        ...
        upcoming: upcoming,
        upcomingTitle:
            page.querySelectorAll('div.widget')[5].querySelector('div.widget-title > div.title')!.text.trim());
  }
  ...
  Future<List<Anime>> getSearchList(final String title) async {
    final document = parse((await getPage(AnimeWorldEndPoints.searchPageUrl + title, _customHeaders)).data);
    return document.querySelectorAll('div.film-list > div.item').map(getAnime).toList(growable: false);
  }
  ...
  Future<AnimeWorldSpecificAnime> getSpecificAnimePage(final String url) async {
    ...
    final infoDd = animePage
        .querySelector('#main > div > div.widget.info > div > div > div.info.col-md-9 > div.row')!
        .querySelectorAll('dd');
      ...
      server.add(AnimeWorldServer(
          ...
          listEpisode: e.querySelectorAll('a').map((episode) {
            ...
            return AnimeWorldEpisode(
                ...
                referer: AnimeWorldEndPoints.sitePrefixNoS + episode.attributes['href']!);
          }).toList(growable: false)));
    }
    return AnimeWorldSpecificAnime(
        image: animePage.querySelector('#thumbnail-watch > img')!.attributes['src']!,
        ...
        comment: AnimeWorldComment(
          referer: AnimeWorldEndPoints.sitePrefixNoS + response.realUri.toString(),
          token: animePage.querySelector('meta#csrf-token')!.attributes['content']!,
          commentId: animePage.querySelector('#player')?.attributes['data-anime-id'] ??
              animePage.querySelector('#loveButton')!.attributes['data-id']!,
        ),
        description: animePage.querySelector('div.desc')?.text.replaceAll(RegExp(r'\s+'), ' ') ??
        ...
        info: DetailAnime(
            ...
            studio:
                infoDd[4].querySelectorAll('a').map((e) => Href(e.text.trim(), e.attributes['href'] ?? '')).toList(),
        ...
        servers: server,
        simili: animePage.querySelectorAll('div.film-list > div.item').map(getAnime).toList(growable: false),
        correlati: animePage.querySelectorAll('div.related > div.item').map(getAnime).toList(growable: false),
  ...
  Future<List<UserComment>> getComment(final AnimeWorldComment info,
    ...
    final request = await _dio.post('${AnimeWorldEndPoints.apiComment}${info.commentId}/$episodeID',
        options: Options(
          headers: head,
        ));
    ...
    return page
        ...
        .map((final comment) => UserComment(
              image: comment.querySelector('img.comment-author-image')!.attributes['src']!,
              text: comment
  ...
  Future<DirectUrlVideo> getUrlVideo(final AnimeWorldEpisode episode, final ServerName nameServer) async {
    var head = Map<String, String>.from(_customHeaders);
    head['X-Requested-With'] = 'XMLHttpRequest';
    head['Referer'] = episode.referer;
    final json = Map<String, String>.from(
        (await _dio.get(AnimeWorldEndPoints.apiEpisode + episode.dataID, options: Options(headers: head))).data as Map);
  ...
  Future<AnimeGenericData> getGenericPageInfo(final String url) async {
    ...
    return AnimeGenericData(_getGenericAnimeList(page), int.parse(page.querySelector('.total')?.text ?? '1'));
  }
  ...
  List<News> _getListNews(final List<Element> elements) {
    return elements
        .map((final element) => News(
              ...
              type: element.querySelectorAll('span.badge.badge-primary').map((final e) => e.text).join(', '),
            ))
        .toList();
  }
  ...
  Future<NewsData> getNewsWithPage(final String url) async {
    final data = (await getPage(url, _customHeaders)).data;
    final page = parse(data);
    return NewsData(_getListNews(page.querySelectorAll('div.post-list > div.item.row')),
